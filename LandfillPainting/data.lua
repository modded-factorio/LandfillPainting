...
if
  mods["angelssmelting"]
  and data.raw.technology["angels-water-washing-1"]
  and (data.raw.technology["angels-water-washing-1"].enabled == nil or data.raw.technology["angels-water-washing-1"].enabled)
  and data.raw.recipe["angels-solid-mud-landfill"]
then
...
for _, v in ipairs(terrains) do
  local item = {
    ...
    place_as_tile = {
      result = v,
      condition_size = 1,
      condition = tile_collision_masks.ground(),
    },
  }

  local recipe = util.table.deepcopy(baserecipe)
  recipe.name = "landfill-" .. v
  recipe.localised_name = { "tile-name." .. v }
  ...
  data.raw.tile[v].minable = {
    mining_time = data.raw.tile["landfill"].minable.mining_time,
    result = item.name,
  }
...
data.raw.item["landfill"].place_as_tile.condition = tile_collision_masks.ground()
